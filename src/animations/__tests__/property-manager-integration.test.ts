...
describe("PropertyManager - Integration Tests", () => {
  ...
  describe("multi-element coordination", () => {
    it("should handle independent animations on multiple elements", () => {
      const animations = [
        ...
        { prop: "backgroundColor" as CSSPropertyName, value: createValue.rgb(255, 128, 0, 1) },
      ];
    ...
    it("should handle mixed CSS and transform properties", () => {
      const transforms: TransformPropertyName[] = ["translateX", "translateY", "scaleX", "rotateZ"];
      const cssProps: CSSPropertyName[] = ["width", "height", "opacity", "backgroundColor"];

      // Apply transforms to first half, CSS to second half
      managers.slice(0, 2).forEach((manager, index) => {
        const transform = transforms[index];
        const value = transform === "scaleX" ? createValue.numeric(1.5, "") 
                     : transform === "rotateZ" ? createValue.numeric(45, "deg")
      ...
      managers.slice(2).forEach((manager, index) => {
        ...
        const value = prop === "opacity" ? createValue.numeric(0.8, "")
                     : prop === "backgroundColor" ? createValue.rgb(0, 255, 128, 1)
    ...
    it("should handle cascading property dependencies", () => {
      ...
      managers.forEach((manager, index) => {
        ...
        manager.updateProperty("borderRadius", createValue.numeric(size / 2, "px"));
      });
  ...
  describe("complex animation scenarios", () => {
    it("should simulate a complete card flip animation", { timeout: 10000 }, () => {
      const manager = managers[0];
      const element = elements[0];
      ...
      const animateFrame = () => {
        if (frameIndex < keyframes.length) {
          ...
          manager.updateProperty("rotateY", createValue.numeric(frame.rotateY, "deg"));
          manager.updateProperty("scaleX", createValue.numeric(frame.scaleX, ""));
          manager.updateProperty("opacity", createValue.numeric(frame.opacity, ""));
    ...
    it("should handle morphing animation with mixed properties", { timeout: 10000 }, () => {
      const manager = managers[0];
      const element = elements[0];
    ...
    it("should handle staggered multi-element animation", { timeout: 15000 }, () => {
      const staggerDelay = 100;
      const animationDuration = 300;
      
      const animations = {
        width: { from: createValue.numeric(100, "px"), to: createValue.numeric(150, "px") },
        height: { from: createValue.numeric(100, "px"), to: createValue.numeric(80, "px") },
        translateY: { from: createValue.numeric(0, "px"), to: createValue.numeric(-20, "px") },
        opacity: { from: createValue.numeric(1, ""), to: createValue.numeric(0.6, "") },
      };
          ...
          Object.entries(animations).forEach(([prop, { from, to }]) => {
            ...
            for (let step = 0; step <= steps; step++) {
              setTimeout(() => {
                ...
                const interpolated = manager.interpolate(prop as any, from, to, progress);
                manager.updateProperty(prop as any, interpolated);
      ...
      const totalTime = (managers.length - 1) * staggerDelay + animationDuration + 100;
  ...
  describe("performance and batching integration", () => {
    it("should efficiently handle many simultaneous property updates", () => {
      ...
      const properties: Array<CSSPropertyName | TransformPropertyName> = [
        ...
        "translateX", "translateY", "scaleX", "rotateZ"
      ];
      ...
      managers.forEach((manager, managerIndex) => {
        properties.forEach((prop, propIndex) => {
          ...
          if (prop === "opacity") {
            ...
            value = createValue.numeric(100 + managerIndex * 20 + propIndex * 10, 
                    prop.includes("translate") ? "px" : (prop.includes("width") || prop.includes("height") ? "px" : ""));
          }
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            elements.forEach((element, index) => {
              ...
              expect(element.style.transform).toContain(`${100 + index * 20 + 40}px`); // translateX
            });
            resolve();
          }, 50);
        }, 10);
    ...
    it("should handle rapid sequential updates efficiently", { timeout: 10000 }, () => {
      const manager = managers[0];
      const element = elements[0];
    ...
    it("should maintain performance with mixed batched and immediate updates", { timeout: 10000 }, () => {
      const batchedManager = new PropertyManager(elements[0], { batchUpdates: true });
      const immediateManager = new PropertyManager(elements[1], { batchUpdates: false });

      const startTime = performance.now();

      // Apply updates to both types
      for (let i = 0; i < 50; i++) {
        batchedManager.updateProperty("width", createValue.numeric(100 + i, "px"));
        immediateManager.updateProperty("width", createValue.numeric(100 + i, "px"));
      }
  ...
  describe("state management and caching integration", () => {
    ...
    it("should handle reset correctly with multiple property types", { timeout: 10000 }, () => {
      const manager = managers[0];
      const element = elements[0];
  ...
  describe("error handling and recovery integration", () => {
    it("should handle mixed success and failure scenarios", { timeout: 10000 }, () => {
      ...
      const operations = [
        ...
        { prop: "backgroundColor" as CSSPropertyName, value: createValue.rgb(255, 0, 128, 1) },
        { prop: "translateX" as TransformPropertyName, value: createValue.numeric(50, "px") },
      ];

      operations.forEach(({ prop, value }) => {
        expect(() => workingManager.updateProperty(prop, value)).not.toThrow();
        expect(() => failingManager.updateProperty(prop, value)).not.toThrow();
      });
    ...
    it("should maintain consistency during partial failures", { timeout: 10000 }, () => {
      const manager = managers[0];
      const element = elements[0];
      ...
      const properties = ["width", "height", "opacity", "borderRadius", "backgroundColor", "color"];
      properties.forEach((prop, index) => {
        const value = prop === "opacity" ? createValue.numeric(0.8, "")
                     : prop.includes("color") ? createValue.rgb(100 + index * 20, 150, 200, 1)
                     : createValue.numeric(120 + index * 10, "px");
        
        expect(() => manager.updateProperty(prop as any, value)).not.toThrow();
      });
  ...
  describe("real-world integration patterns", () => {
    it("should simulate a complex dashboard widget animation", () => {
      ...
      const widgetAnimations = {
        chart: {
          width: { from: createValue.numeric(100, "px"), to: createValue.numeric(180, "px") },
          height: { from: createValue.numeric(100, "px"), to: createValue.numeric(120, "px") },
          backgroundColor: { from: createValue.rgb(100, 150, 200, 1), to: createValue.rgb(120, 180, 220, 1) },
          scaleY: { from: createValue.numeric(1, ""), to: createValue.numeric(1.05, "") },
        },
        metric: {
          opacity: { from: createValue.numeric(0.8, ""), to: createValue.numeric(1, "") },
          translateY: { from: createValue.numeric(0, "px"), to: createValue.numeric(-5, "px") },
          borderRadius: { from: createValue.numeric(8, "px"), to: createValue.numeric(12, "px") },
        },
        progress: {
          width: { from: createValue.numeric(100, "px"), to: createValue.numeric(160, "px") },
          rotateZ: { from: createValue.numeric(0, "deg"), to: createValue.numeric(2, "deg") },
          backgroundColor: { from: createValue.rgb(255, 200, 100, 1), to: createValue.rgb(255, 220, 120, 1) },
        },
        status: {
          scaleX: { from: createValue.numeric(1, ""), to: createValue.numeric(0.95, "") },
          scaleY: { from: createValue.numeric(1, ""), to: createValue.numeric(1.1, "") },
          opacity: { from: createValue.numeric(1, ""), to: createValue.numeric(0.9, "") },
        },
      };

      // Apply animations with different timing
      widgets.forEach((widget, widgetIndex) => {
        const animations = widgetAnimations[widget.type as keyof typeof widgetAnimations];
        ...
        setTimeout(() => {
          ...
          for (let step = 0; step <= steps; step++) {
            setTimeout(() => {
              ...
              Object.entries(animations).forEach(([prop, { from, to }]) => {
                const interpolated = widget.manager.interpolate(prop as any, from, to, progress);
    ...
    it("should handle responsive animation adjustments", { timeout: 15000 }, () => {
      const manager = managers[0];
      const element = elements[0];
      ...
      const animateViewportChange = () => {
        if (currentViewport < viewports.length) {
          ...
          manager.updateProperty("width", createValue.numeric(baseWidth * viewport.multiplier, "px"));
          manager.updateProperty("height", createValue.numeric(baseHeight * viewport.multiplier, "px"));
          manager.updateProperty("translateX", createValue.numeric(baseTransform * viewport.multiplier, "px"));
          manager.updateProperty("scaleX", createValue.numeric(viewport.multiplier, ""));
    ...
    it("should simulate a loading sequence with multiple elements", () => {
      ...
      managers.forEach((manager, index) => {
        ...
        manager.updateProperty("opacity", createValue.numeric(0.7 + index * 0.1, ""));
        manager.updateProperty("width", createValue.numeric(120 + index * 10, "px"));
      });
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            elements.forEach((element, index) => {
              ...
              expect(parseFloat(element.style.opacity)).toBeCloseTo(parseFloat(expectedOpacity), 1);
