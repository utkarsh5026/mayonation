...
describe("StyleAnimator - Edge Cases", () => {
  ...
  describe("special numeric values", () => {
    it("should handle Infinity values", () => {
      ...
      return new Promise<void>((resolve) => {
        requestAnimationFrame(() => {
          expect(element.style.width).toBe("Infinitypx");
          resolve();
        });
      });
    });
    ...
    it("should handle NaN values", () => {
      ...
      return new Promise<void>((resolve) => {
        requestAnimationFrame(() => {
          expect(element.style.width).toBe("NaNpx");
          resolve();
        });
      });
    });
    ...
    it("should handle interpolation with special values", () => {
      ...
      const result = animator.interpolate("width", normalValue, infinityValue, 0.5);
  ...
  describe("color edge cases", () => {
    ...
    it("should handle color interpolation with zero alpha", () => {
      ...
      const result = animator.interpolate("backgroundColor", transparentRed, opaqueBlue, 0.5);
    ...
    it("should handle HSL color edge cases", () => {
      ...
      const result = hslAnimator.interpolate("color", boundaryHue, zeroHue, 0.5);
      expect(result.type).toBe("color");
    });
  ...
  describe("precision edge cases", () => {
    ...
    it("should handle very high precision", () => {
      const highPrecisionAnimator = new StyleAnimator(element, { precision: 20 });
      const value = createValue.numeric(1.23456789012345678901234567890, "px");
  ...
  describe("memory and performance edge cases", () => {
    ...
    it("should handle massive batch updates", () => {
      const properties: CSSPropertyName[] = ["width", "height", "opacity", "borderRadius"];
    ...
    it("should handle memory cleanup under stress", () => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        animator.applyAnimatedPropertyValue("height", createValue.numeric(i, "px"));
  ...
  describe("browser compatibility edge cases", () => {
    ...
    it("should handle detached elements", () => {
      ...
      expect(() => {
        ...
        detachedAnimator.applyAnimatedPropertyValue("width", createValue.numeric(100, "px"));
    ...
    it("should handle CSS property name variations", () => {
      ...
      testCases.forEach(prop => {
        expect(() => {
          animator.applyAnimatedPropertyValue(prop, createValue.rgb(255, 0, 0, 1));
        }).not.toThrow();
      });
    });
  });
  ...
  describe("animation timing edge cases", () => {
    it("should handle rapid animation frame requests", () => {
      ...
      const rapidAnimation = () => {
        if (frameCount < maxFrames) {
          frameCount++;
          animator.applyAnimatedPropertyValue("width", createValue.numeric(frameCount * 10, "px"));
    ...
    it("should handle batched updates with timing edge cases", () => {
      ...
      for (let i = 0; i < 50; i++) {
        animator.applyAnimatedPropertyValue("width", createValue.numeric(i, "px"));
        animator.applyAnimatedPropertyValue("height", createValue.numeric(i * 2, "px"));
      }
